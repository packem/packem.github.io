...
- All plugins **MUST** fall under the `plugins` field in the config, relative to `root`, `output` and `transformer`.
...
- **Plugins are executed in order of definition (FIFE &mdash; First In, First Executed)**. The order of plugin definition plays a big role in how you would want Packem to behave. Packem executes your plugins in sequentially (i.e. in order of definition) to make debugging much less of a pain.
