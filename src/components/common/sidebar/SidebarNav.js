            ...
            {sidebarfile.groups.map((group, i) => (
                <div key={i} className="mt1">
                    {group.items.some(item => (item.link === location.pathname)) ?
                        ...
                        : <h4 className="f5 fw5 link pa0 ma0">{(group.items[0].link ? <SidebarLink link={group.items[0].link} title={group.group} linkClasses="midgrey hover-yellow-d3 link" /> : group.group)}</h4>}
                </div>
            ))}
        </nav>
    )
